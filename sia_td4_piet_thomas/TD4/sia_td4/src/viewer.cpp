            ...
            for(int i = 0; i < camera->sampleCount(); i++){
                for(int j = 0; j < camera->sampleCount(); j++){
                    ...
                    if(camera->isStratified()){
                        ...
                        rd1 = Eigen::internal::random<float>(0,1);
                        rd2 = Eigen::internal::random<float>(0,1);

                    }

                    ray.direction = (camF + camX * (2.f*float(x + i/camera->sampleCount() + rd1/camera->sampleCount() + offset[0])/float(camera->vpWidth())  - 1.f) 
                                    - camY * (2.f*float(y + j/camera->sampleCount() + rd2/camera->sampleCount() + offset[1])/float(camera->vpHeight()) - 1.f)).normalized();
