...
void SpringForce::addForces() {
    // TODO: implement this
    Vector3d l = p1->x - p0->x;

    Vector3d f_mass_r = -ks * (l.norm() - l0) * (l / l.norm());
    Vector3d f_mass_a = -kd * ((p1->v - p0->v).dot(l) / l.norm()) * (l / l.norm());
    Vector3d f = f_mass_r + f_mass_a;
...
void makeGrid(ParticleSystem *psys) {
    ...
    double dy_diag = sqrt(dx * dx + dy * dy);
        ...
        for (int j = 0; j <= n; j++) {
            ...
            if(i < m){
                if(j < n){
                    ...
                    psys->forces.push_back(new SpringForce(p0, p1, ks, d, dy_diag));
                }
                if(j > 0){
                    Particle *p1 = particles[i+1][j-1];
                    psys->forces.push_back(new SpringForce(p0, p1, ks, d, dy_diag));
