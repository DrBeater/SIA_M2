...
void Viewer::init(int w, int h) {
  ...
  _lightPos = Vector4f(1.f, 0.75f, 1.f, 1.f);
  _pointLight = Sphere(0.025f);
  _pointLight.init();
  _pointLight.setTransformationMatrix(
      Affine3f(Translation3f(_lightPos.head(3))));
  _lightColor = Vector3f(1.f, 1.f, 1.f);

 
  Mesh* shadowMesh = mesh->computeShadowVolume(_lightPos.head(3));
...
void Viewer::display() {
  ...
  glUniform4fv(_blinnPrg.getUniformLocation("light_pos_view"), 1,
  ...
  for (uint i = 0; i < _shapes.size(); ++i) {
    glUniformMatrix4fv(_blinnPrg.getUniformLocation("model_matrix"), 1,
                       GL_FALSE, _shapes[i]->getTransformationMatrix().data());
  ...
  for(uint i=0; i<_shadowShapes.size(); ++i) {
    glUniformMatrix4fv(_simplePrg.getUniformLocation("model_matrix"), 1, GL_FALSE, 
                       _shadowShapes[i]->getTransformationMatrix().data());
