...
void Viewer::loadScene(const std::string& filename)
{
    if(filename.size()>0) {
        ...
        if (root->getClassType() == ::Object::EScene){
            ...
            for(unsigned int i=0; i<m_scene->lightList().size(); ++i) {
...
void renderBlock(Scene* scene, ImageBlock& block)
{
    ...
    unsigned int nbSample = camera->sampleCount();
...
void render(Scene* scene, ImageBlock* result, std::string outputName, bool* done)
{
    ...
    std::cout << "Raytracing time : " << timer.elapsedString() << " -  nb triangle intersection: " << Mesh::ms_itersection_count << "\n";
