...
Color3f Ward::brdf(const Vector3f& viewDir, const Vector3f& dir, const Normal3f& normal, const Vector2f& uv) const
{
    ...
    Color3f diffuse = m_diffuseColor * (1.0/M_PI);
    if(coef > 1e-8f)
...
float Ward::pdf(const Normal3f &n, const Vector3f &i, const Vector3f &o) const
{
   ...
   float diffuseProb = (1.0/M_PI) * o.dot(n);
...
Vector3f Ward::is(const Normal3f& n, const Vector3f& i, float& pdf) const
{
    ...
    Vector3f x = n.unitOrthogonal(); //Vector3f(0,1,0) - Vector3f(0,1,0).dot(n)*n;
    x.normalize();
    Vector3f y = n.cross(x);
    Vector3f dir;
    ...
    if (chooseSpecular) {
        ...
        // Or compute cos(phiH) and sin(phiH) directly with formulas 43-44 from "Importance Sampling for Production Rendering"
        // float cos_v = std::cos(2.f * M_PI * v);
        // float sin_v = std::sin(2.f * M_PI * v);
        // float coef_x = m_alphaX * cos_v;
        // float coef_y = m_alphaY * sin_v;
        // float denom = std::sqrt(coef_x*coef_x + coef_y*coef_y);
        // float cos_phiH = m_alphaX * cos_v / denom;
        // float sin_phiH = m_alphaY * sin_v / denom;
        ...
        dir = d.x() * x + d.y() * y + d.z() * n;
    }
    
    pdf = this->pdf(n,i,dir);
